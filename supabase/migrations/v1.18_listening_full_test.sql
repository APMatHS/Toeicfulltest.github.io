-- TOEIC Full Test V1.18 — Listening / Full Test foundation
-- Backward compatible with existing Reading tests.
...
begin
  ...
  if v_id is null then
    ...
    values(nullif(p_data->>'class_id','')::uuid,p_data->>'title',p_data->>'description',coalesce(nullif(p_data->>'duration_minutes','')::int,75),coalesce(nullif(p_data->>'max_attempts','')::int,1),coalesce(nullif(p_data->>'status','')::public.test_status,'draft'),coalesce(nullif(p_data->>'show_answers_after_submit','')::boolean,false),coalesce(nullif(p_data->>'anti_cheat_mode','')::public.anti_cheat_mode,'warn_then_submit'),coalesce(nullif(p_data->>'allowed_violations','')::int,2),nullif(p_data->>'opens_at','')::timestamptz,nullif(p_data->>'closes_at','')::timestamptz,auth.uid(),v_new_kind)
    returning id into v_id;
    insert into public.audit_logs(actor_id,action,target_type,target_id,metadata) values(auth.uid(),'test_created','test',v_id::text,coalesce(p_data,'{}'::jsonb));
  else
    ...
    update public.tests set
      ...
      test_kind=v_new_kind
    where id=v_id;
    insert into public.audit_logs(actor_id,action,target_type,target_id,metadata) values(auth.uid(),'test_updated','test',v_id::text,coalesce(p_data,'{}'::jsonb));
  end if;
  return v_id;
end
$$;
...
begin
 ...
 if v_id is null then
   ...
   select test_id into v_test_id from public.test_parts where id=v_id;
   perform public._assert_test_content_editable(v_test_id);
   update public.test_parts set
     title=coalesce(p_data->>'title',title),
     shuffle_mode=coalesce(p_data->>'shuffle_mode',shuffle_mode),
...
begin
  ...
  return jsonb_build_object(
    ...
    'parts',coalesce((select jsonb_agg(jsonb_build_object('id',tp.id,'part_no',tp.part_no,'title',tp.title,'shuffle_mode',tp.shuffle_mode,'shuffle_choices',tp.shuffle_choices,'sort_order',tp.sort_order) order by tp.sort_order) from public.test_parts tp where tp.test_id=p_test_id),'[]'::jsonb),
    'questions',coalesce((select jsonb_agg(jsonb_build_object('id',q.id,'part_no',tp.part_no,'test_part_id',q.test_part_id,'source_number',q.source_number,'source_order',q.source_order,'content',q.content,'correct_choice_key',q.correct_choice_key,'score_weight',q.score_weight,'stimulus_group_id',q.stimulus_group_id,'media_type',q.media_type,'storage_path',q.storage_path,'choices',(select jsonb_agg(jsonb_build_object('key',qc.choice_key,'content',qc.content,'media_type',qc.media_type,'storage_path',qc.storage_path) order by qc.choice_key) from public.question_choices qc where qc.question_id=q.id)) order by tp.sort_order,q.source_order) from public.questions q join public.test_parts tp on tp.id=q.test_part_id where tp.test_id=p_test_id),'[]'::jsonb),
    'stimulus_groups',coalesce((select jsonb_agg(jsonb_build_object('id',sg.id,'part_no',tp.part_no,'test_part_id',sg.test_part_id,'source_order',sg.source_order,'title',sg.title,'play_mode',sg.play_mode,'allow_replay',sg.allow_replay,'allow_seek',sg.allow_seek,'max_plays',sg.max_plays,'stimuli',(select jsonb_agg(jsonb_build_object('id',s.id,'media_type',s.media_type,'content',s.content,'storage_path',s.storage_path,'sort_order',s.sort_order) order by s.sort_order) from public.stimuli s where s.stimulus_group_id=sg.id)) order by tp.sort_order,sg.source_order) from public.stimulus_groups sg join public.test_parts tp on tp.id=sg.test_part_id where tp.test_id=p_test_id),'[]'::jsonb)
  );
end
$$;
...
begin
 ...
 return jsonb_build_object(
 'attempt',jsonb_build_object('id',v_a.id,'test_id',v_a.test_id,'attempt_no',v_a.attempt_no,'status',v_a.status,'started_at',v_a.started_at,'expires_at',v_a.expires_at,'violation_count',v_a.violation_count,'anti_cheat_mode',v_mode,'allowed_violations',v_allowed,'test_kind',v_kind,'test_title',v_title),
 'questions',coalesce((select jsonb_agg(jsonb_build_object(
 'id',q.id,'number',aq.display_number,'order',aq.display_order,'content',q.content,'part',tp.part_no,'shuffle_choices',tp.shuffle_choices,'stimulus_group_id',q.stimulus_group_id,'media_type',q.media_type,'storage_path',q.storage_path,
 'group_settings',(select jsonb_build_object('play_mode',sg.play_mode,'allow_replay',sg.allow_replay,'allow_seek',sg.allow_seek,'max_plays',sg.max_plays) from public.stimulus_groups sg where sg.id=q.stimulus_group_id),
 'stimuli',coalesce((select jsonb_agg(jsonb_build_object('id',s.id,'type',s.media_type,'content',s.content,'storage_path',s.storage_path) order by s.sort_order) from public.stimuli s where s.stimulus_group_id=q.stimulus_group_id),'[]'::jsonb),
 'choices',(select jsonb_agg(jsonb_build_object('key',qc.choice_key,'content',qc.content,'media_type',qc.media_type,'storage_path',qc.storage_path) order by qc.choice_key) from public.question_choices qc where qc.question_id=q.id),
 'selected',(select a.selected_choice_key from public.answers a where a.attempt_id=p_attempt_id and a.question_id=q.id),'marked',coalesce((select a.is_marked_review from public.answers a where a.attempt_id=p_attempt_id and a.question_id=q.id),false)) order by aq.display_order)
 from public.attempt_questions aq join public.questions q on q.id=aq.question_id join public.test_parts tp on tp.id=q.test_part_id where aq.attempt_id=p_attempt_id),'[]'::jsonb));
end
$$;
...
begin
  ...
  return coalesce((select jsonb_agg(jsonb_build_object('unit_key',s.unit_key,'started_at',s.started_at,'last_position_seconds',s.last_position_seconds,'completed_at',s.completed_at,'updated_at',s.updated_at) order by s.started_at) from public.attempt_audio_states s where s.attempt_id=p_attempt_id),'[]'::jsonb);
end
$$;
...
begin
  ...
  update public.attempt_audio_states set
    ...
    updated_at=now()
  where attempt_id=p_attempt_id and unit_key=p_unit_key;
  select * into v_s from public.attempt_audio_states where attempt_id=p_attempt_id and unit_key=p_unit_key;
  return jsonb_build_object('unit_key',v_s.unit_key,'started_at',v_s.started_at,'last_position_seconds',v_s.last_position_seconds,'completed_at',v_s.completed_at,'updated_at',v_s.updated_at);
end
$$;
...
begin
  ...
  with audio_units as (
    select 'question:'||q.id::text unit_key
    from public.questions q join public.test_parts tp on tp.id=q.test_part_id
    where tp.test_id=v_a.test_id and tp.part_no between 1 and 4 and q.media_type='audio' and q.storage_path is not null
    union
    select 'stimulus:'||s.id::text
    ...
    where tp.test_id=v_a.test_id and tp.part_no between 1 and 4 and s.media_type='audio' and s.storage_path is not null
  )
  select count(*)::int into v_missing from audio_units u
  where not exists(select 1 from public.attempt_audio_states st where st.attempt_id=p_attempt_id and st.unit_key=u.unit_key and st.completed_at is not null);
...
declare
  ...
  v_required_ok boolean; v_standard boolean; v_label text; v_audio_parts int:=0;
begin
  if public.current_role() not in ('teacher','system_admin') then raise exception 'Forbidden'; end if;
  ...
  select count(*)::int into v_students from public.class_members cm join public.profiles p on p.id=cm.user_id where cm.class_id=v_t.class_id and p.role='student' and p.is_active;
  select count(distinct part_no)::int into v_audio_parts from (
    ...
    union
    select tp.part_no from public.stimuli s join public.stimulus_groups sg on sg.id=s.stimulus_group_id join public.test_parts tp on tp.id=sg.test_part_id
    where tp.test_id=p_test_id and tp.part_no between 1 and 4 and s.media_type='audio' and s.storage_path is not null
  ) audio_parts;

  if v_t.test_kind='listening' then
    v_required_ok:=v_p1>0 and v_p2>0 and v_p3>0 and v_p4>0 and v_audio_parts=4;
    ...
    v_required_ok:=v_p1>0 and v_p2>0 and v_p3>0 and v_p4>0 and v_p5>0 and v_p6>0 and v_p7>0 and v_audio_parts=4;
  ...
  return jsonb_build_object(
    ...
    'structure_complete',v_required_ok,'structure_standard',v_standard,'structure_label',v_label,'listening_audio_parts',v_audio_parts,
    'reading_standard',(v_t.test_kind='reading' and v_standard),
...
as $$
declare
  s public.tests%rowtype; new_test uuid; sp public.test_parts%rowtype; np uuid; sg public.stimulus_groups%rowtype; ng uuid; q public.questions%rowtype; nq uuid; mapped_group uuid;
begin
  if public.current_role() not in ('teacher','system_admin') then raise exception 'Forbidden'; end if;
  select * into s from public.tests where id=p_source_test_id;
  if not found then raise exception 'Source test not found'; end if;
  insert into public.tests(class_id,title,description,duration_minutes,max_attempts,status,show_answers_after_submit,anti_cheat_mode,allowed_violations,opens_at,closes_at,created_by,test_kind)
  values(case when p_overrides?'class_id' then nullif(p_overrides->>'class_id','')::uuid else s.class_id end,coalesce(nullif(p_overrides->>'title',''),s.title||' - Bản sao'),coalesce(p_overrides->>'description',s.description),coalesce(nullif(p_overrides->>'duration_minutes','')::int,s.duration_minutes),coalesce(nullif(p_overrides->>'max_attempts','')::int,s.max_attempts),'draft',coalesce(nullif(p_overrides->>'show_answers_after_submit','')::boolean,s.show_answers_after_submit),coalesce(nullif(p_overrides->>'anti_cheat_mode','')::public.anti_cheat_mode,s.anti_cheat_mode),coalesce(nullif(p_overrides->>'allowed_violations','')::int,s.allowed_violations),nullif(p_overrides->>'opens_at','')::timestamptz,nullif(p_overrides->>'closes_at','')::timestamptz,auth.uid(),s.test_kind)
  returning id into new_test;
  create temporary table if not exists tmp_group_map(old_id uuid primary key,new_id uuid) on commit drop;
  ...
  for sp in select * from public.test_parts where test_id=p_source_test_id order by sort_order loop
    ...
    for q in select * from public.questions where test_part_id=sp.id order by source_order loop
      mapped_group:=null;
      if q.stimulus_group_id is not null then select new_id into mapped_group from tmp_group_map where old_id=q.stimulus_group_id; end if;
