...
export function createListeningPracticeApp(ctx){
  ...
  async function renderAttempt(testId,{mode="listening",onComplete=null}={}){
    ...
    const ui=readJSON(uiKey(attemptId),{current:0}),attempt={...practice.attempt,test_title:authoring?.test?.title||"TOEIC Listening",violation_count:0};
    state={attemptId,preview:true,testId,payload:{test:authoring?.test,attempt,questions},current:Math.min(ui.current||0,questions.length-1),saveStatus:start?.resumed?"Đã phục hồi lượt làm thử":"Lượt làm thử đã được lưu",audioStates:new Map(),activeAudio:null,mode,onComplete,pendingSave:null};
  ...
  async function moveTo(index){if(!state||index<0||index>=state.payload.questions.length)return;const target=state.payload.questions[index];if(state.activeAudio&&!audio.sameActiveUnit(target))return toast("Audio đang phát. Chỉ có thể chuyển giữa các câu dùng chung audio hiện tại.",5000);saveAttemptUi();state.current=index;await media.ensureQuestionMedia(target);if(!state||state.current!==index)return;draw();media.prefetchQuestion(index+1);}

  function draw(){
    if(!state)return;getView().innerHTML=renderListeningView(state,{audio,media});const q=state.payload.questions[state.current],unit=questionAudioUnit(q);
    document.querySelector("#playListeningAudio")?.addEventListener("click",()=>audio.begin(unit,q));document.querySelectorAll('input[name="answer"]').forEach(input=>input.onchange=()=>saveCurrent(input.value,!!q.marked));const mark=document.querySelector("#markListening");if(mark)mark.onchange=e=>saveCurrent(q.selected,!!e.target.checked);document.querySelector("#prevListening")?.addEventListener("click",()=>moveTo(state.current-1));document.querySelector("#nextListening")?.addEventListener("click",()=>state.current===state.payload.questions.length-1?completeListening():moveTo(state.current+1));document.querySelectorAll(".listening-palette button[data-idx]").forEach(btn=>btn.onclick=()=>moveTo(Number(btn.dataset.idx)));document.querySelector("#submitListening")?.addEventListener("click",()=>state.mode==="full"?completeListening():confirmFinish());updateTimer();
  }
