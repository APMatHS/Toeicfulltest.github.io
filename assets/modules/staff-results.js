...
export function createStaffResultsController({sb,esc,fmt,statusBadge,toast,getWorkspace,setLiveChannel,clearLiveChannel,refreshCurrentTest}){
  ...
  async function confirmAttemptAction(action,testId,attemptId,name){
    ...
    const close=()=>modalRoot.innerHTML="";modalRoot.querySelector("[data-close]").onclick=close;modalRoot.querySelector("#doAttemptAction").onclick=async()=>{const fn=isDelete?"staff_delete_attempt":"staff_reset_attempt";const {data:result,error}=await sb.rpc(fn,{p_attempt_id:attemptId});if(error)return toast(error.message,6000);close();if(isDelete){toast(result?.content_unlocked?"Đã xóa bài làm · đề đϣ được mở khóa":"Đã xóa bài làm");await refreshCurrentTest(testId,"submissions");return;}toast("Đã reset lượt làm");const w=getWorkspace();if(w?.id===testId){w.loaded.submissions=false;w.loaded.live=false;}clearLiveChannel();await renderSubmissionsTab(testId);};
  }
  ...
  async function exportTestExcel(testId){
    ...
    const {data,error}=await sb.rpc("staff_get_test_export",{p_test_id:testId});if(error)return toast(error.message,6000);const students=data.students||[],testKind=data.test?.test_kind||"reading";
    const summary=students.map((s,i)=>{
      const answers=s.answers||[],total=answers.length,stats=sectionStats(answers),base={STT:i+1,"Họ tên":s.full_name,MSSV:s.student_code||"","Lần làm":s.attempt_no||"","Trạng thái":s.status||"Chưa làm","Bắt đầu":s.started_at?new Date(s.started_at).toLocaleString("vi-VN"):"","Nộp bài":s.submitted_at?new Date(s.submitted_at).toLocaleString("vi-VN"):""};
    ...
    const detail=[];for(const s of students)for(const a of s.answers||[]){const part=Number(a.part)||"",section=part>=1&&part<=4?"Listening":part>=5&&part<=7?"Reading":"";detail.push({"Họ tên":s.full_name,MSSV:s.student_code||"","Lần làm":s.attempt_no||"",Part:part,"Section":section,"Câu":a.number,"Đã chọn":a.selected||"","Đáp án":a.correct||"","Đúng/Sai":a.selected?(a.is_correct?"Đúng":"Sai"):"Chưa trả lời"});}
