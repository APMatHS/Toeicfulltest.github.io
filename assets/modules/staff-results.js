...
export function createStaffResultsController({sb,esc,fmt,statusBadge,toast,getWorkspace,setLiveChannel,clearLiveChannel,refreshCurrentTest}){
  ...
  async function renderLiveTab(testId){
    const load=async()=>{
      ...
      const counts={roster:classRows.length,attempts:classRows.filter(r=>r.attempt_id).length,in:classRows.filter(r=>r.status==="in_progress").length,done:classRows.filter(r=>["submitted","auto_submitted"].includes(r.status)).length,none:classRows.filter(r=>!r.attempt_id).length,viol:classRows.filter(r=>(r.violation_count||0)>0).length};
