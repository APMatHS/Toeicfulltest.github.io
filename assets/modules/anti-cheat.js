...
export function createAntiCheatController({
  ...
  function violationLimit(){
    const mode=getExamState?.()?.payload?.attempt?.anti_cheat_mode;
    return mode==="strict"?1:3;
  }
  ...
  function updateOverlay(state){
    ...
    if(state.returned){
      ...
      el.querySelector("[data-ac-meta]").textContent=limit===1?`Vi phạm: ${count}/1 · Chế độ nghiêm ngặt tự nộp ở vi phạm đầu tiên`:`Vi phạm: ${count}/3 · Lần thứ 3 sẽ tự động nộp`;
      el.hidden=false;
    ...
    if(message) message.textContent=limit===1?"Chế độ nghiêm ngặt: rời màn hình sẽ tự động nộp bài.":"Mỗi lần rời màn hình được tính ngay là 1 vi phạm. Quá 15 giây hoặc vi phạm lần thứ 3, bài sẽ tự động nộp.";
    if(returnBtn) returnBtn.hidden=true;
    el.querySelector("[data-ac-meta]").textContent=`Vi phạm: ${count}/${limit} · Rời quá 15 giây sẽ tự động nộp`;
    el.hidden=false;
  }
  ...
  async function endAway(){
    ...
    // Không tự đóng cảnh báo và không dùng toast thay thế.
