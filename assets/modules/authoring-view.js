...
function questionRow(q,locked=false){
  ...
  return `<div class="question-author-row author-question" id="author-q-${q.source_number}" data-number="${q.source_number}" data-search="${search}">
    ...
    ${locked?"":`<div class="row wrap question-actions"><button class="secondary sm add-question-after" data-part="${q.test_part_id}" data-partno="${q.part_no}" data-after="${q.source_number}" data-group="${q.stimulus_group_id||""}">+ Thêm sau</button><button class="secondary sm edit-question" data-id="${q.id}">Sửa</button><button class="ghost sm duplicate-question" data-id="${q.id}">Nhân bản</button><button class="danger sm delete-question" data-id="${q.id}" data-number="${q.source_number}">Xóa</button></div>`}
  </div>`;
}
    ...
    <div class="authoring-shell"><div class="authoring-main">
      ${parts.map(p=>{
        ...
        return `<details class="part-editor author-part" data-search="part ${p.part_no} ${esc(p.title||"").toLowerCase()}" open>
          ...
          ${pGroups.map(g=>{const gQs=groupQuestions(g,pQs);const nums=gQs.map(q=>Number(q.source_number)).filter(Number.isFinite).sort((a,b)=>a-b);const range=nums.length?(nums.length===1?`Câu ${nums[0]}`:`Câu ${nums[0]}–${nums.at(-1)}`):"Chưa có câu";return `<details class="group-box author-group" data-id="${g.id}" data-search="${esc(`${g.title||""} ${g.source_order} ${(g.stimuli||[]).map(s=>plainText(s.content||"")).join(" ")}`.toLowerCase())}" open><summary><div class="row between wrap"><div><b>${esc(g.title||`Nhóm ${g.source_order}`)}</b><div class="muted">${range} · ${gQs.length} câu · ${esc(g.play_mode||"normal")}</div></div>${locked?"":`<div class="row wrap"><button class="ghost sm add-stimulus" data-group="${g.id}">+ Nội dung chung</button><button class="ghost sm edit-group" data-id="${g.id}">Sửa nhóm</button><button class="danger sm delete-group" data-id="${g.id}">Xóa nhóm</button></div>`}</div></summary><div class="group-body"><div class="author-group-content">${(g.stimuli||[]).map(s=>`<div class="author-stimulus-full"><div class="author-stimulus-head"><span class="badge">${s.media_type==="image"?"🖼 Ảnh":s.media_type==="audio"?"🔊 Audio":"📝 Văn bản"}</span>${locked?"":`<div><button class="ghost xs edit-stimulus" data-group="${g.id}" data-id="${s.id}">Sửa</button><button class="danger xs delete-stimulus" data-id="${s.id}">Xóa</button></div>`}</div>${mediaBlock(s)}</div>`).join("")||'<div class="muted">Chưa có nội dung chung</div>'}</div>${gQs.map(q=>questionRow(q,locked)).join("")||'<div class="muted mini-empty">Chưa có câu trong nhóm</div>'}</div></details>`;}).join("")}
