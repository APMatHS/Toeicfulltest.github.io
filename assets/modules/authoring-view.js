...
function questionHasIssue(q){
  ...
  return !q.correct_choice_key || choices.length<4 || choices.some(c=>!plainText(c.content||"")&&!c.storage_path) || (!plainText(q.content||"")&&!q.storage_path&&q.part_no===5);
}
    ...
    <div class="authoring-shell">
      ...
      ${parts.map(p=>{
        ...
        return `<details class="part-editor author-part" data-search="part ${p.part_no} ${esc(p.title||"").toLowerCase()}" open>
          ...
          ${pGroups.map(g=>{
            const gQs=groupQuestions(g,pQs); const nums=gQs.map(q=>Number(q.source_number)).filter(Number.isFinite).sort((a,b)=>a-b);
            const range=nums.length?(nums.length===1?`Câu ${nums[0]}`:`Câu ${nums[0]}–${nums.at(-1)}`):"Chưa có câu";
            return `<details class="group-box author-group" data-id="${g.id}" data-search="${esc(`${g.title||""} ${g.source_order} ${(g.stimuli||[]).map(s=>plainText(s.content||"")).join(" ")}`.toLowerCase())}" open>
              <summary><div class="row between wrap"><div><b>${esc(g.title||`Nhóm ${g.source_order}`)}</b><div class="muted">${range} · ${gQs.length} câu · ${esc(g.play_mode||"normal")}</div></div>
              ...
              <div class="group-body"><div class="author-group-content">${(g.stimuli||[]).map(s=>`<div class="author-stimulus-full"><div class="author-stimulus-head"><span class="badge">${s.media_type==="image"?"🖼 Ảnh":s.media_type==="audio"?"🔊 Audio":"📝 Văn bản"}</span>${locked?"":`<div><button class="ghost xs edit-stimulus" data-group="${g.id}" data-id="${s.id}">Sửa</button><button class="danger xs delete-stimulus" data-id="${s.id}">Xóa</button></div>`}</div>${mediaBlock(s)}</div>`).join("")||'<div class="muted">Chưa có nội dung chung</div>'}</div>
              ${gQs.map(q=>questionRow(q,locked)).join("")||'<div class="muted mini-empty">Chưa có câu trong nhóm</div>'}
              </div></details>`;
          }).join("")}
      ...
      <aside class="authoring-nav" aria-label="Điều hướng câu hỏi"><h3>Câu hỏi</h3><div class="authoring-nav-summary"><span class="badge">${questions.length} câu</span><span class="badge">${issueCount} cần kiểm tra</span></div>
      ${parts.map(p=>{const pQs=questions.filter(q=>q.part_no===p.part_no).sort((a,b)=>Number(a.source_number)-Number(b.source_number));return `<div class="authoring-nav-part"><div class="authoring-nav-part-title">${esc(p.title||`Part ${p.part_no}`)}</div><div class="authoring-nav-grid">${pQs.map(q=>`<button type="button" class="author-q-jump ${questionHasIssue(q)?"issue":""}" data-target="author-q-${q.source_number}" title="Câu ${q.source_number}">${q.source_number}</button>`).join("")}</div></div>`}).join("")}
      </aside>
    </div>
  </section>`;
}
...
export function bindAuthoringFilter(root=document){
  ...
  root.querySelectorAll(".author-q-jump").forEach(btn=>btn.addEventListener("click",()=>{
    const target=root.querySelector(`#${CSS.escape(btn.dataset.target)}`); if(!target) return;
    target.closest(".author-part")?.setAttribute("open",""); target.closest(".author-group")?.setAttribute("open","");
    root.querySelectorAll(".author-q-jump.active").forEach(x=>x.classList.remove("active")); btn.classList.add("active");
