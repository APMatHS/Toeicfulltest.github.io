...
export function createQuestionBankListController(ctx){
  const {sb,toast,getSession,getView}=ctx;
  let requestSeq=0;

  function storageKey(){return `toeic.questionBank.filters.${getSession()?.user?.id||"anon"}`;}
  function loadState(){
    const saved=readJSON(storageKey(),{});return {...DEFAULT_STATE,...saved,page:Math.max(1,Number(saved?.page)||1)};
  }
  let state=loadState();
  ...
  function filterShell(){
    return `<div class="form-grid" id="bankFilters">
      <label class="span-2">Tìm kiếm
        <input id="bankSearch" type="search" value="${esc(state.search)}" placeholder="Mã câu, tiêu đề, dạng câu hoặc nguồn..." autocomplete="off">
      </label>
      <label>Part
        <select id="bankPart"><option value="">Tất cả Part</option>${Array.from({length:7},(_,i)=>`<option value="${i+1}" ${String(i+1)===String(state.part)?"selected":""}>Part ${i+1}</option>`).join("")}</select>
      </label>
      <label>Đơn vị
        <select id="bankItemType"><option value="">Tất cả</option><option value="Q" ${state.itemType==="Q"?"selected":""}>Câu đơn</option><option value="G" ${state.itemType==="G"?"selected":""}>Nhóm</option></select>
      </label>
      <label>Trạng thái
        <select id="bankStatus"><option value="">Tất cả</option><option value="draft" ${state.status==="draft"?"selected":""}>Nháp</option><option value="approved" ${state.status==="approved"?"selected":""}>Đã duyệt</option><option value="inactive" ${state.status==="inactive"?"selected":""}>Ngừng dùng</option></select>
      </label>
      <label>Độ khó
        <select id="bankDifficulty"><option value="">Tất cả</option><option value="unrated" ${state.difficulty==="unrated"?"selected":""}>Chưa xếp</option><option value="easy" ${state.difficulty==="easy"?"selected":""}>Dễ</option><option value="medium" ${state.difficulty==="medium"?"selected":""}>Trung bình</option><option value="hard" ${state.difficulty==="hard"?"selected":""}>Khó</option></select>
      </label>
    </div>`;
  }
  ...
  function renderRows(items,total){
    ...
    const rows=items.map(item=>`<tr>
      ...
      <td>${esc(item.usage_count||0)}<div class="small muted">${item.last_used_at?`Gần nhất: ${esc(fmt(item.last_used_at))}`:"Chưa sử dụng"}</div></td>
    ...
    return `<div class="row between wrap" style="margin-top:16px"><div class="muted small">${total||0} mục · ${PAGE_SIZE} mục/trang</div><div class="row"><button class="ghost sm" id="bankPrev" ${state.page<=1?"disabled":""}>← Trước</button><span class="small"><b>${state.page}</b> / ${totalPages}</span><button class="ghost sm" id="bankNext" ${state.page>=totalPages?"disabled":""}>Sau →</button></div></div>
      <div class="table-wrap"><table><thead><tr><th>Mã</th><th>Phân loại</th><th>Trạng thái</th><th>Nội dung / tag</th><th>Nguồn</th><th>Đã dùng</th><th>Cập nhật</th></tr></thead><tbody>${rows||`<tr><td colspan="7" class="empty">Chưa có câu hỏi phù hợp bộ lọc.</td></tr>`}</tbody></table></div>
  ...
  async function loadSummary(){
    const counts=await Promise.all(["", "draft","approved","inactive"].map(status=>{
      let q=sb.from("bank_items").select("id",{count:"exact",head:true});if(status)q=q.eq("status",status);return q;
    }));
    const val=i=>counts[i].error?"—":String(counts[i].count||0);
    const el=getView().querySelector("#bankSummary");
  ...
  async function loadItems({keepPage=false}={}){
    ...
    let query=sb.from("bank_items").select("id,public_code,item_type,part_no,status,difficulty,title,primary_type,tags,topics,source_type,source_name,usage_count,last_used_at,updated_at",{count:"exact"});
    ...
    host.innerHTML=renderRows(data||[],total);bindPagination(totalPages);saveState();
  }

  function bindPagination(totalPages){
    const view=getView();
    const move=delta=>{const next=Math.min(totalPages,Math.max(1,state.page+delta));if(next===state.page)return;state.page=next;saveState();loadItems({keepPage:true});window.scrollTo({top:0,behavior:"smooth"});};
  ...
  function bindFilters(){
    const view=getView();
    const refresh=()=>{state.page=1;saveState();loadItems();};
    const search=view.querySelector("#bankSearch");
    const debounced=debounce(()=>{state.search=search.value.trim();refresh();},350);
    if(search)search.addEventListener("input",debounced);
    [["bankPart","part"],["bankItemType","itemType"],["bankStatus","status"],["bankDifficulty","difficulty"]].forEach(([id,key])=>{
      const el=view.querySelector(`#${id}`);if(el)el.onchange=()=>{state[key]=el.value;refresh();};
    });
    const reload=view.querySelector("#bankReload");if(reload)reload.onclick=()=>{loadSummary();loadItems({keepPage:true});};
    const reset=view.querySelector("#bankResetFilters");if(reset)reset.onclick=()=>{state={...DEFAULT_STATE};saveState();renderBank();};
  }

  async function renderBank(){
    state=loadState();const view=getView();
    view.innerHTML=`<section class="card"><div class="row between wrap"><div><h1>Ngân hàng câu hỏi</h1><p class="muted">Quản lý câu hỏi TOEIC theo Part, trạng thái, độ khó, dạng câu và lịch sử sử dụng.</p></div><div class="row wrap"><button id="bankResetFilters" class="ghost">Xóa bộ lọc</button><button id="bankReload" class="secondary">Làm mới</button></div></div><div id="bankSummary" class="row wrap" style="margin:12px 0 18px"><span class="badge">Đang tải thống kê...</span></div>${filterShell()}<div id="bankList"></div></section>`;
