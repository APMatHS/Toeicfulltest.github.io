...
export function createQuestionBankGeneratorController(ctx){
  ...
  function baseWarnings(){
    const out=[];for(const p of state.parts){const part=Number(p.part_no??p),capacity=state.index.filter(x=>Number(x.part_no)===part).reduce((n,x)=>n+itemQuestionCount(x),0),need=Number(state.targets[part])||0;if(capacity<need)out.push(`Part ${part}: ngân hàng khả dụng chỉ có ${capacity}/${need} câu cần thêm.`);}return out;
  }
  ...
  async function showPreview(id){
    try{state.previewDetail=(await loadDetails([id]))[0]||null;render();}catch(err){toast(`Không xem được item: ${err.message}`,6500);}
  }
  ...
  function fillRest(){
    const oldLocks=new Map(state.selected.map(x=>[x.item.id,x.locked])),keep=state.selected.map(x=>({...x,locked:true})),result=autoFill(state.index,state.parts,state.targets,state.prefs,keep,state.excluded);
    state.selected=result.selected.map(x=>({...x,locked:oldLocks.get(x.item.id)??false}));syncWarnings(result.warnings);state.currentPart=nextIncompletePart(state.parts,state.targets,state.selected);refreshCandidates();
  }

  function pick(id){
    const item=state.index.find(x=>x.id===id);if(!item)return;
    if(state.replacingId){
      const pos=state.selected.findIndex(x=>x.item.id===state.replacingId);if(pos<0)return;
      const old=state.selected[pos];if(old.locked)return toast("Hãy mở khóa item trước khi thay.");if(itemQuestionCount(item)!==itemQuestionCount(old.item))return toast("Item thay thế phải có cùng số câu để giữ ma trận.");
      state.excluded.add(old.item.id);state.selected[pos]={item,locked:false};state.replacingId=null;
    }else{
      const left=remaining(item.part_no);if(itemQuestionCount(item)>left)return toast(`Part ${item.part_no} chỉ còn thiếu ${left} câu.`);
      state.selected.push({item,locked:false});
    }
  ...
  async function open({testId,parts=[],questions=[]}){
    ...
    try{
      const {data:test,error}=await sb.from("tests").select("id,title,test_kind,content_locked_at").eq("id",testId).single();if(error)throw error;if(test.content_locked_at)throw new Error("Đề đã khóa nội dung nên không thể thêm câu từ ngân hàng.");
