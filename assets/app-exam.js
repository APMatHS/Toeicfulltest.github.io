...
export function createExamApp(ctx){
  ...
  async function renderStudent(){
    ...
    const [{data:tests=[],error},{data:attempts=[]}]=await Promise.all([sb.from("tests").select("*,classes(name)").eq("status","published").is("archived_at",null).order("created_at",{ascending:false}),sb.from("attempts").select("*").eq("student_id",session.user.id).order("attempt_no",{ascending:false})]);
    if(error)console.error(error);const byTest={};for(const a of attempts)(byTest[a.test_id]??=[]).push(a);
    view.innerHTML=`<section class="card"><h1>Bài kiểm tra của tôi</h1><p class="muted">${esc(profile.full_name)} · ${esc(profile.student_code||"")}</p></section><section class="grid grid-2 student-tests">${tests.map(t=>{const all=byTest[t.id]||[],valid=all.filter(a=>a.status!=="reset"),inProgress=valid.find(a=>a.status==="in_progress"),latest=inProgress||valid.sort((a,b)=>(b.attempt_no||0)-(a.attempt_no||0))[0],used=valid.length,remaining=Math.max(0,(t.max_attempts||1)-used);let action="";if(inProgress)action=`<a class="btn primary" href="#/exam/${inProgress.id}">Tiếp tục lượt ${inProgress.attempt_no||1}</a>`;else if(remaining>0)action=`<button class="primary start-test" data-id="${t.id}" data-max="${t.max_attempts||1}" data-used="${used}">${used?`Làm lượt ${used+1}`:"Bắt đầu"}</button>${latest?`<a class="btn secondary" href="#/result/${latest.id}">Xem lượt trước</a>`:""}`;else if(latest)action=`<a class="btn secondary" href="#/result/${latest.id}">Xem kết quả</a>`;return `<div class="card"><div class="row between"><div class="row wrap"><span class="badge">${esc(t.classes?.name||"TOEIC")}</span><span class="badge">${esc(testKindShortLabel(t.test_kind))}</span></div>${latest?statusBadge(latest.status):'<span class="status off">Chưa làm</span>'}</div><h2>${esc(t.title)}</h2><p class="muted">${esc(t.description||"Bài kiểm tra TOEIC")}</p><div class="row wrap"><span>⏱ ${t.duration_minutes} phút</span><span>•</span><span>${used}/${t.max_attempts||1} lượt đã dùng</span>${remaining?`<span>• còn ${remaining}</span>`:""}</div><div class="test-action row wrap">${action}</div></div>`;}).join("")||'<div class="card empty">Hiện chưa có bài kiểm tra được mở.</div>'}</section>`;
