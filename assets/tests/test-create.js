...
export function createTestCreateController(ctx){
  ...
  const classOptions=classes=>`<option value="">Chưa gán lớp</option>${classes.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join("")}`;
  ...
  async function open(){
    ...
    modalRoot.innerHTML=`<div class="modal-backdrop"><div class="modal wide create-test-modal">
      <div class="row between wrap"><div><h2>Tạo bài kiểm tra</h2><p class="muted">Listening, Reading và Full Test dùng chung một workspace; chức năng được tách module để dễ bảo trì.</p></div><button class="ghost sm" data-close>Đóng</button></div>
      ...
      <section class="create-mode-panel" data-mode-panel="import" hidden>
        <form id="importTestForm" class="form-grid">
          ...
          <label>Lớp<select name="class_id">${options}</select></label>
  ...
  function bindClone(){const form=modalRoot.querySelector("#cloneTestForm"),source=form.elements.source_test_id;source.onchange=()=>{const o=source.selectedOptions[0];if(!o?.value)return;form.elements.title.value=`${o.textContent.split(" · ")[0]} - Bản sao`;form.elements.duration_minutes.value=o.dataset.duration||75;form.elements.max_attempts.value=o.dataset.max||1;form.elements.class_id.value=o.dataset.class||"";};form.onsubmit=async e=>{e.preventDefault();const f=Object.fromEntries(new FormData(form)),sourceId=f.source_test_id;delete f.source_test_id;f.opens_at=f.opens_at?toIso(f.opens_at):"";f.closes_at=f.closes_at?toIso(f.closes_at):"";const btn=e.submitter||form.querySelector('button[type="submit"]');btn.disabled=true;btn.textContent="Đang nhân bản…";const {data,error}=await sb.rpc("staff_clone_test_v118b",{p_source_test_id:sourceId,p_overrides:f});btn.disabled=false;btn.textContent="Tạo bản sao";if(error)return toast(error.message,6000);invalidate();closeModal();toast("Đã tạo bài từ bài kiểm tra cũ");go(`/test/${data}`);};}
